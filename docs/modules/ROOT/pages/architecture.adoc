...
`RuntimeServices` now defines the shared session-lifetime composition root for API client, catalog, archive, and favorites collaborators. `RuntimeWiring` remains as the thin adapter for older callers while Phase 7 moves TUI and headless entry paths onto the explicit bundle shape.
