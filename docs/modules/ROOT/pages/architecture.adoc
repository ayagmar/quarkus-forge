...
`RuntimeServices` now defines the shared session-lifetime composition root for API client, catalog, archive, and favorites collaborators. `TuiBootstrapService` consumes that bundle for interactive and smoke sessions, while `RuntimeWiring` remains the thin adapter for the remaining headless and CLI assembly steps.
