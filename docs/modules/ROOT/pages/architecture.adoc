...
Startup policy now lives under `application/`: `LiveStartupMetadataLoader` owns the live-metadata timeout and snapshot-fallback path, while `DefaultStartupStateService` merges requested prefill, stored prefill, and compiled defaults before handing the result to `InputResolutionService`.
