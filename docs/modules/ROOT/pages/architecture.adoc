...
Within `headless/`, `HeadlessGenerationService` is now narrowing toward orchestration-only responsibility: extension/preset reconciliation lives in `HeadlessExtensionResolutionService`, while catalog transport stays in `HeadlessCatalogClient` and generation execution remains delegated to the project generator path.
