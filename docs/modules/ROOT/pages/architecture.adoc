...
CoreTuiController (Orchestration Shell)
    ...
    └── Extracted Helpers
            ...
            ├── CatalogLoadCoordinator       Async catalog lifecycle + startup overlay timing
...
| *Encapsulation*
| `PostGenerationMenuState`, `CatalogLoadCoordinator`, and `MetadataSelectorManager` each own their state and expose it through focused APIs.
