...
`RuntimeWiring` is the single composition owner for persistence-backed stores plus shared API and archive services. Headless orchestration now resolves favorites through `persistence` and depends on a catalog loader plus project generator rather than pulling `RuntimeConfig` into `HeadlessGenerationService`.
...
Reducer-visible UI semantics are now converged for form and extension read-model state. `ProjectRequest`, `ValidationReport`, and extension search/filter/read-model summaries update through reducer intents/effects and live in reducer-owned `UiState`, while generation progress and startup overlay lines are still synchronized from controller-owned runtime collaborators before render. `UiStateSnapshotMapper` only adds render-only panel/footer snapshots plus the effective status line. Mutable widget internals such as `TextInputState` and `ListState` stay in adapter/runtime context rather than `UiState`.
...
Render-only `UiState` fields:

* `metadataPanel`, `extensionsPanel`, `footer`
* `generation`, `startupOverlay`

The render-only fields above are synchronized from runtime collaborators immediately before render. They are not reducer authority and are the first candidates to move into a dedicated render model in the next slices of this phase.

`UiRenderModel` is now the explicit container for those render-only slices while the codebase transitions away from storing them directly on `UiState`.
