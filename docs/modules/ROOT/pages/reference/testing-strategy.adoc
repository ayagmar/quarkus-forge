...
* `./mvnw verify -DskipTests=true` (integration-only) does not publish JaCoCo site reports.
* `./mvnw clean verify` publishes both reports:
** Unit HTML/XML: `target/site/jacoco/index.html` and `target/site/jacoco/jacoco.xml`
** Integration HTML/XML: `target/site/jacoco-it/index.html` and `target/site/jacoco-it/jacoco.xml`
