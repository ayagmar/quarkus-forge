...
CI runs both docs build and docs link checking as separate verification steps. Docs Pages uses the same shared entrypoints when publishing the site. CI also builds and smoke-tests the interactive Windows JVM jar plus the interactive Windows native executable so the shared terminal bootstrap path is exercised before release.
