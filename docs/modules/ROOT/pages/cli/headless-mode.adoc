...
Save a reusable recipe by name into `~/.quarkus-forge/recipes`:
...
java -jar target/quarkus-forge.jar generate \
  ...
  --build-tool maven \
  --java-version 25 \
  --write-recipe starter.json
----

Reuse that recipe later by name:

[source,bash]
----
java -jar target/quarkus-forge.jar generate --recipe starter.json
----
...
* `--recipe <name>` falls back to `~/.quarkus-forge/recipes/<name>` when no local file exists.
* `--write-recipe <name>` writes to `~/.quarkus-forge/recipes/<name>` when `<name>` has no path separators.
