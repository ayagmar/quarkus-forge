...
Headless mode is designed for repeatable generation in CI, scripts, build agents, and containers.
...
NOTE: All examples above also work with the full jar (`quarkus-forge.jar`). The headless jar is the better default for CI and containers because it avoids TUI and terminal dependencies entirely.
...
GitHub releases publish a matching `.sha256` file for every headless JVM jar and headless native binary.

Verify a downloaded headless release artifact after downloading both the artifact and its `.sha256` file:

[source,bash]
----
sha256sum -c quarkus-forge-headless.jar.sha256
...
[source,bash]
----
shasum -a 256 -c quarkus-forge-headless.jar.sha256
...
Generate checksum files for locally built release artifacts:
...
Verify a locally built headless jar:
...
* Source mode is surfaced in diagnostics (`live`, `cache`, `snapshot`). This is informational and shows where data came from: `live` means fetched from upstream in real time, `cache` means served from a local cache, and `snapshot` means served from a bundled immutable snapshot.
