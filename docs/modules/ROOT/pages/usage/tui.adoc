...
The TUI is optimized for interactive project setup, extension discovery, and fast keyboard-driven generation.
...
[NOTE]
====
Post-generation actions are convenience handoffs after scaffold creation.
Shell-based actions still run as local commands on the host machine.
====
...
[WARNING]
====
`QUARKUS_FORGE_IDE_COMMAND` is executed through the local shell.
Use only trusted command strings.
====
