...
**Issue**: #10  
**Branch target**: `feat/headless-cli`  
...
| Pre-requisite package refactoring (favorites store) | New headless features (JSON output deferred to [`json-output-spec.md`](json-output-spec.md)) |
...
> **Note**: The remaining ~50 files in `ui/` have NO tamboui imports. Some must move to
...
AsyncFailureHandler.java        ← QuarkusForgeCli.EXIT_CODE_CANCELLED (×2) + EXIT_CODE_NETWORK (×1)
...
```
QuarkusForgeGenerateCommandTest.java      ← QuarkusForgeCli.EXIT_CODE_* (9 occurrences)
HeadlessGenerationServiceTest.java        ← QuarkusForgeCli.EXIT_CODE_NETWORK + CommandLine.ExitCode.OK
QuarkusForgeCliStartupMetadataTest.java   ← QuarkusForgeCli.EXIT_CODE_VALIDATION (2 occurrences)
ProjectRequestFactoryTest.java            ← QuarkusForgeCli.EXIT_CODE_* (4 occurrences)
```
...
ExitCodes (new utility class, package-private)
  └── OK=0, VALIDATION=2, NETWORK=3, ARCHIVE=4, CANCELLED=130
  └── (no USAGE constant — picocli ExitCode.USAGE=2 == VALIDATION=2; use ExitCodes.VALIDATION)
```
...
- `dev/ayagmar/quarkusforge/ShellExecutor.java`
- `dev/ayagmar/quarkusforge/ShellProcessRunner.java`
- `dev/ayagmar/quarkusforge/ShellExecutorDiagnostics.java`
...
Update all import sites (search with: `rg "quarkusforge\.ui\.(ExtensionFavoritesStore|FileBackedExtension|InMemoryExtension|ExtensionFavoritesPayload|ExtensionFavoriteIds)" --include="*.java"`):
- Affected files with explicit imports: `HeadlessGenerationService.java`, `TuiBootstrapService.java`,
  `QuarkusForgeGenerateCommandTest.java`.

> **Same-package (no-import) sites** — the rg command above does NOT find these because they
> access `ExtensionFavoritesStore` from within the `ui` package without an explicit import:
> - `ui/CoreTuiController.java` — add `import dev.ayagmar.quarkusforge.api.ExtensionFavoritesStore;`
> - `ui/ExtensionCatalogState.java` — add same import
> - `src/test/.../ui/CoreTuiControllerFactorySignatureTest.java` — add same import
> - `src/test/.../ui/CoreTuiExtensionSearchPilotTest.java` — add same import
>
> Also move `ExtensionFavoritesStoreTest.java` (see special case below).

> **`ExtensionFavoritesStoreTest` special case**: this test is in the
> `dev.ayagmar.quarkusforge.ui` package and uses `ExtensionFavoritesStore` without an
> explicit import (same-package access). The rg command above will NOT find it.
> Move the test file from
> `src/test/java/dev/ayagmar/quarkusforge/ui/ExtensionFavoritesStoreTest.java`
> to `src/test/java/dev/ayagmar/quarkusforge/api/ExtensionFavoritesStoreTest.java`
> and update its `package` declaration to `dev.ayagmar.quarkusforge.api`.
...
rg "quarkusforge\.ui\.(ExtensionFavoritesStore|FileBackedExtensionFavoritesStore|InMemoryExtensionFavoritesStore|ExtensionFavoritesPayload|ExtensionFavoriteIds)" src/
# must return 0 results (all favorites classes now in api/)
rg "import dev.ayagmar.quarkusforge.ui" \
    src/main/java/dev/ayagmar/quarkusforge/RuntimeConfig.java \
    src/main/java/dev/ayagmar/quarkusforge/HeadlessGenerationService.java
# both must return 0 results (QuarkusForgeCli still uses ui.UserPreferencesStore — that is correct)
```
...
final class ExitCodes {
    ...
    static final int VALIDATION = 2;   // bad user input; matches picocli ExitCode.USAGE = 2
...
> **Why no `USAGE` constant**: picocli's `CommandLine.ExitCode.USAGE = 2` and the existing
> `EXIT_CODE_VALIDATION = 2` are the same value — both represent "bad user input". Defining
> a separate `USAGE = 1` (SOFTWARE error in picocli terms) would silently change behavior
> and break existing tests (which assert exit code 2 for validation failures). All "exit 2"
> paths consolidate under `ExitCodes.VALIDATION`.

**Changes to production code:**
- `QuarkusForgeCli`: remove the 4 `EXIT_CODE_*` constant definitions (they move to `ExitCodes`);
  replace `CommandLine.ExitCode.USAGE` → `ExitCodes.VALIDATION` (2 occurrences)
  and `CommandLine.ExitCode.OK` → `ExitCodes.OK` (3 occurrences); keep the
  `import picocli.CommandLine` (still needed for `new CommandLine(...)` in `runWithArgs()`)
...
- `QuarkusForgeCliStartupMetadataTest`: replace `QuarkusForgeCli.EXIT_CODE_VALIDATION` → `ExitCodes.VALIDATION` (2 occurrences)
- `ProjectRequestFactoryTest`: replace all `QuarkusForgeCli.EXIT_CODE_*` → `ExitCodes.*` (4 occurrences)
...
- `configureTerminalBackendPreference()` — private (only called from `run()`)
- `isBackendPreferenceExplicitlyConfigured()` — private (only called from `configureTerminalBackendPreference()`)

Move as **package-private** (to preserve test access):
- `appTuiConfig()` — package-private static method on `TuiBootstrapService`
- `appBindingsProfile()` — package-private static method on `TuiBootstrapService`
- `defaultBackendPreference()` — package-private static method on `TuiBootstrapService`

Move constants to `TuiBootstrapService` (private):
- `BACKEND_PROPERTY_NAME`, `BACKEND_ENV_NAME`, `PANAMA_BACKEND` (used by the moved methods above)
- `TUI_TICK_RATE` (used by `appTuiConfig()`)

Move constant to `TuiBootstrapService` (package-private, since it is currently accessed
package-wide in `QuarkusForgeCli` and tests may reference it in future):
- `STARTUP_SPLASH_MIN_DURATION`

`TuiBootstrapService.run()` currently calls these via `QuarkusForgeCli.*` — after the move
they will be accessed directly (internal calls or `this`-less static references).

> **`catalogLoadDiagnostics()` stays in `QuarkusForgeCli`**: `TuiBootstrapService.run()`
> also calls `QuarkusForgeCli.catalogLoadDiagnostics()`, but that method is used internally
> by `QuarkusForgeCli.runHeadlessSmoke()` too, so it stays. Both files are excluded from
> the headless profile, so the cross-reference is fine.

Remove the now-unused `dev.tamboui.tui.TuiConfig` and `dev.tamboui.tui.bindings.Bindings` imports
from `QuarkusForgeCli`. Also remove the now-unused static fields from `QuarkusForgeCli`:

> **New import needed in `TuiBootstrapService`**: after moving `appBindingsProfile()` (which
> returns `dev.tamboui.tui.bindings.Bindings`), add `import dev.tamboui.tui.bindings.Bindings;`
> to `TuiBootstrapService.java`. The `TuiConfig` import is already present.
`TUI_TICK_RATE`, `STARTUP_SPLASH_MIN_DURATION`, `BACKEND_PROPERTY_NAME`, `BACKEND_ENV_NAME`,
`PANAMA_BACKEND` (all move to `TuiBootstrapService`).

> **Do NOT remove `TUI_BOOTSTRAP_SERVICE`** — it is still used by `QuarkusForgeCli.runTui()`
> which delegates to `TUI_BOOTSTRAP_SERVICE.run(...)`. Remove only the moved constants listed
> above; `TUI_BOOTSTRAP_SERVICE`, `SHELL_EXECUTOR`, and `POST_TUI_ACTION_EXECUTOR` remain in
> `QuarkusForgeCli`.

**Update tests that reference the moved methods:**
- `QuarkusForgeCliBindingsProfileTest`: replace `QuarkusForgeCli.appTuiConfig()` →
  `TuiBootstrapService.appTuiConfig()` (2 occurrences) and `QuarkusForgeCli.appBindingsProfile()` →
  `TuiBootstrapService.appBindingsProfile()` (3 occurrences)
- `QuarkusForgeCliTest`: replace `QuarkusForgeCli.defaultBackendPreference()` →
  `TuiBootstrapService.defaultBackendPreference()` (1 occurrence)
...
rg "QuarkusForgeCli\.appTuiConfig\|QuarkusForgeCli\.appBindingsProfile\|QuarkusForgeCli\.defaultBackendPreference" src/test/
# must return 0 results (replaced with TuiBootstrapService.*)
...
- `QuarkusForgeCli.runHeadlessGenerate()`: change signature from
  `Integer runHeadlessGenerate(GenerateCommand command)` →
  `public int runHeadlessGenerate(GenerateCommand command)`
  (Java does not allow `Integer` to satisfy an interface's `int` return type — they are
  distinct JVM descriptors. Both visibility and return type must change.)
...
>
> **How to verify AP succeeded**: check that
> `target/classes/META-INF/native-image/picocli-generated/` contains the `reflect-config.json`
> descriptor after `mvn compile`.
>
...
### Phase 4 — HeadlessCli entry point + CliVersionProvider fix

**`CliVersionProvider` fix** (must do before Phase 5):

`CliVersionProvider.resolveVersion()` currently calls `QuarkusForgeCli.class.getPackage().getImplementationVersion()`. Because `QuarkusForgeCli` is excluded from the headless compile, this will fail with "cannot find symbol" when compiling the headless profile. Fix: replace with `CliVersionProvider.class.getPackage()` — same Java package, same manifest entry, identical runtime result.
      ...
      <plugin>
        ...
        <configuration>
          <excludes>
            ...
            <exclude>dev/ayagmar/quarkusforge/ShellExecutor.java</exclude>
            <exclude>dev/ayagmar/quarkusforge/ShellProcessRunner.java</exclude>
            <exclude>dev/ayagmar/quarkusforge/ShellExecutorDiagnostics.java</exclude>
          </excludes>
        </configuration>
      </plugin>
    </plugins>
...
**Verification loop:**
```bash
# 1. Compile only (fast check — must use clean to avoid stale classes from default build)
mvn -Pheadless clean compile -q

# 2. Build jar
mvn -Pheadless clean package -q
...
# exit 0, prints "Dry-run validated successfully:" + field summary
...
> **Why not just `-Pheadless,native`?** After Phase 1B, `-Pheadless,native` works too — the
> existing `native` profile uses `${forge.main.class}` and `${forge.jar.name}`, so the headless
> overrides take effect. The only difference is `-H:+SharedArenaSupport` (needed for tamboui's
> Panama backend), which is present in `native` but omitted in `headless-native`. This flag is
> harmless when tamboui is absent but adds unnecessary native-image metadata. If the extra profile
> feels like over-engineering, use `-Pheadless,native` directly and remove Phase 7 entirely.
...
>
> **Resource-config cleanup**: the hand-written
> `src/main/resources/META-INF/native-image/dev.ayagmar/quarkusforge/resource-config.json`
> includes patterns for `ui/.*\\.tcss$` and `dev/tamboui/tui/bindings/.*\\.properties$`.
> These are TUI-only resources. In the headless native build, tamboui jars are still on
> the compile classpath (`provided` scope) and GraalVM would include these resources,
> adding unnecessary bloat (~200 bytes for the `.tcss` file, plus tamboui's bindings
> properties from the jar). Consider either:
> - Creating a headless-specific `resource-config.json` via a profile-activated resource
>   filtering, or
> - Simply leaving it (the bloat is negligible and GraalVM's reachability analysis already
>   excludes unreachable tamboui classes).
>
...
> **Follow-up**: `--format json` structured output for the `generate` command is specified
> separately in [`docs/json-output-spec.md`](json-output-spec.md). It builds on Phase 6
> and can be implemented independently after the headless profile is complete.
...
- [ ] `rg "quarkusforge\.ui\.(ExtensionFavoritesStore|FileBackedExtension|InMemoryExtension|ExtensionFavoritesPayload|ExtensionFavoriteIds)" src/` returns 0 results (Pre-Phase 1 done)
...
- [ ] `rg "QuarkusForgeCli" src/main/java/dev/ayagmar/quarkusforge/CliVersionProvider.java` returns 0 results
- [ ] `mvn -Pheadless clean compile -q` succeeds
- [ ] `mvn -Pheadless clean package -q` succeeds
...
- [ ] `java -jar target/quarkus-forge-headless.jar generate --dry-run -g org.acme -a demo` exits 0 (requires network; skip in offline CI)
- [ ] Default build still works: `mvn package -DskipTests -q` produces `quarkus-forge.jar` with tamboui classes (manual-only: `java -jar target/quarkus-forge.jar` launches TUI)
    ...
    → Phase 1 (ExitCodes + Maven properties) → verify
      → Phase 2 (move TUI helpers to TuiBootstrapService) → verify
        → Phase 3 (HeadlessRunner interface + GenerateCommand) → verify
          → Phase 4 (HeadlessCli + CliVersionProvider fix) → verify
...
- No new headless features (JSON output deferred to [`json-output-spec.md`](json-output-spec.md))
